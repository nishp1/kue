...
exports.removeBadJob = function (id, hash) {
    var client = redis.client();
    if( hash && hash.state ) {
        client.zrem('q:jobs:' + hash.state, id);
    }
    client.multi()
        .del('q:job:' + id + ':log')
        .del('q:job:' + id)
...
exports.log = function (id, fn) {
    /*redis*/Job.client/*()*/.lrange('q:job:' + id + ':log', 0, -1, fn);
};
...
Job.prototype.removeState = function (fn) {
    ...
    if( state && state != '' ) {
        client.multi()
            .zrem('q:jobs:' + state, this.id)
            .zrem('q:jobs:' + this.type + ':' + state, this.id)
        .exec( fn );
    } else {
        fn && fn();
    }
...
Job.prototype.state = function (state, fn) {
    ...
    this.set('state', state, function(){
        var multi = client.multi()
            .zadd('q:jobs:' + state, this._priority, this.id)
            .zadd('q:jobs:' + this.type + ':' + state, this._priority, this.id);
...
Job.prototype._getId = function (fn) {
    var self = this;
    if (this.id) {
        // check if job already exist
        self.client.hgetall('q:job:' + this.id, function (err, hash) {
            if (err) {
                fn(err);
            } else if (hash) {
                fn('job already exist');
            } else {
                fn(null, self.id);
            }
        });
        ...
        // incr id
        self.client.incr('q:ids', function (err, id) {
            if (err) return fn(err);
            self.id = id
            fn(err,id)
        });
...
Job.prototype.save = function (fn) {
    ...
    // update
    if (this.saved) return this.update(fn);
    this.saved = true;

    this._getId(function(err, id) {
        if (err) return fn(err);
